 ...
 * @param states
 * @param idx 
 * @param max_allowed_cn_change 
 * @param cn_increase_rate: can be either site-level or chr-level change depending on the input parameters. 
                    * If site-level change, the cn_increase_rate is actually dup_rate; 
                    * if chr-level change, the cn_increase_rate is actually chr_gain_rate.
 * @param cn_decrease_rate: can be either site-level or chr-level change depending on the input parameters. 
                    * If site-level change, the cn_decrease_rate is actually del_rate; 
                    * if chr-level change, the cn_decrease_rate is actually chr_loss_rate.
...
vector<Transition> build_transitions(const vector<pair<int,int>>& states,
    ...
    int mat_tcn = 2 * max_hap_cn;   // maximum total copy number, used to determine the state space
    ...
    for (size_t i = 0; i < states.size(); ++i) {
        ...
        if (a > 0 && a < max_hap_cn && (a + 1 + b) <= mat_tcn) {
            pair<int,int> next = {a + 1, b};
        ...
        if (b > 0 && b < max_hap_cn && (a + b + 1) <= mat_tcn) {
            pair<int,int> next = {a, b + 1};
...
// rate matrix for haplotype-specific segment-level CNAs using independent Markov chain model
// manually fill the entries for now, which is easier to debug and less error-prone than using code to generate the matrix, given the complex patterns of non-zero entries.
...
// rate matrix for haplotype-specific chr-level CNAs using independent Markov chain model
...
void get_rate_matrix_chr_change_haplotype(double* m, double chr_gain_rate, double chr_loss_rate, int max_chr_change_haplotype){
    ...
    m[i + (i + 3) * ncol] = chr_gain_rate;
    m[i + i * ncol] = 0 - chr_loss_rate - chr_gain_rate;

    // the row before the normal state
    i = NO_CHANGE_HAPLOTYPE - 1;
    m[i + (i - 2) * ncol] = chr_loss_rate;
    ...
    // the row after the normal state
    i = NO_CHANGE_HAPLOTYPE + 1;
    m[i + (i - 3) * ncol] = chr_loss_rate;
    m[i + i * ncol] = 0 - chr_loss_rate;

    // the two rows before the last row
